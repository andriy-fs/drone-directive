...
Root scripts: `dev`/`build`/`preview` delegate to `client`; `test` runs `net`, then `chat`, then `client`; `lint` is a single root `eslint .` covering **every** workspace; `type-check` chains `types`/`net`/`chat`/`server`. `deploy` is the whole thing at once (`scripts/deploy.sh`: tests → build → static site → relay → smoke test). Per-workspace extras: `npm run codegen -w protocol`, `npm run deploy -w client` (static site) and `npm run deploy -w server` (relay) — both Cloudflare, separate hostnames, see `.docs/deployment.md` — plus `npm run e2e -w server`. `npm run dev:relay` is a root alias for `npm run dev -w server`. `publish:client` (build → `scripts/pack-dist.mjs` → `npm publish ./client/.pack`) ships the built game to GitHub Packages as `@andriy-fs/drone-directive-client` — static assets for the desktop shell, on a `v*` tag only; see `.docs/deployment.md`.
...
- **Pixi** (`client/src/pixi/**`) — canvas rendering + input. Owns a `GameEngine`; `WorldRenderer` drives views from miniplex **reactive queries**; app-layer adapters subscribe the bus (audio) and push throttled snapshots to the store. Audio is sample files played through `@pixi/sound` (table in `client/src/config/sounds.ts`, files in `client/public/sounds/`, briefs in `.docs/internal/sfx/README.md`) and runs off **three** channels: the bus for what the simulation does (shots, explosions, a robot leaving the factory), a store subscription for what the player picks (selection is store-only state the engine never hears about), and direct calls from `ui/common/` for the interface itself. No React imports.
- **React/UI** (`client/src/ui/**` — `App`, `GameCanvas`, `hud/`, `screens/`, `common/`, `hooks/`, `features/`; plus `client/src/store/**`) — HUD/menus only. Most of `ui/` is cut by **kind** (`common/`, `hooks/`, `hud/`, `screens/`); `features/<name>/` is the single exception cut by **subject**, for a concern whose pure logic, hook and component only ever serve each other — `features/device/` (the screen-size gate) is the one that exists. Talks to the Zustand store; never imports Pixi objects or ECS entities. (`client/src/store/**` stays outside `ui/` — the Pixi bridge reads it too.) The interface's look is tokenised: `client/src/ui/App.css` names no colour, font or radius of its own but reads roles from `client/src/theme/**`, where `tokens.css` holds the base scheme on `:root` and each alternative is one `[data-theme='…']` override file (the *default* the player gets is a separate one-line choice, `DEFAULT_THEME`) — so a new scheme touches no shared stylesheet (`client/src/theme/README.md`). The **battlefield** is not themed: everything Pixi draws still comes from `config/palette.ts`.

Below all three sits one shared layer that is not a layer of its own: **`client/src/models/**`** — every unit's shape as line segments, plus the projection that puts one on a screen. It imports `@drone-directive/types/enums` and nothing else (no Pixi, no React, no store, no `engine/`, no `config/`), because three different consumers need it and none may reach the others: the hull view strokes it into a `Graphics`, `ui/common/Wireframe.tsx` writes it out as SVG, and the field view may want it later through a third camera. See `client/src/models/README.md`.
...
- Workspace-level: `client/README.md` (**the engine↔Pixi↔UI flow, diagrammed** — the four channels into the store, the one React↔Pixi seam, and why `miniplex-react` is the wrong fit here), `protocol/README.md` (BARE schema, codegen, framing), `net/README.md` (transport, codec, validation, injected config), `chat/README.md` (why chat is not in the lockstep stream, and its two departures from `net/`), `types/README.md`.
