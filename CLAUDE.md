...
Root scripts: `dev`/`build`/`preview` delegate to `client`; `test` runs `net`, then `chat`, then `client`; `lint` is a single root `eslint .` covering **every** workspace; `type-check` chains `types`/`net`/`chat`/`server`. `deploy` is the whole thing at once (`scripts/deploy.sh`: tests → build → static site → relay → smoke test). Per-workspace extras: `npm run codegen -w protocol`, `npm run deploy -w client` (static site) and `npm run deploy -w server` (relay) — both Cloudflare, separate hostnames, see `.docs/deployment.md` — plus `npm run e2e -w server`. `npm run dev:relay` is a root alias for `npm run dev -w server`.
