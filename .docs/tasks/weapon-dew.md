...
## Постфактум — ИИ разменивал DEW впустую

Симптом из игры: бот присылал **одиночный** DEW, тот замораживал один юнит и
погибал — размен целого юнита на 8 секунд. Две причины, обе в `systems/ai.ts`:
DEW попадал в общий `rest`/`staged` и уезжал с волной (или один, при
`posture === 'offensive'`), а после выстрела оставался на линии огня все 5 секунд
перезарядки.

Правится **политикой бота**, без новых программ: нужное поведение уже есть в
`Overwatch` («идти позади наступающей группы»). Новый `positionDewUnits` в
`ai.ts` решает каждый тик:

- нет `gameConfig.ai.dewEscortMin` (2) **вооружённых** роботов в наступлении →
  `Guard` у базы. Считаются только `weapon.damage > 0`, поэтому DEW не может
  быть эскортом другому DEW;
- есть эскорт и оружие заряжено → `AttackRobots` (идёт с группой, выбирает цель);
- только что выстрелил (`cooldownLeft > 0`) → `Overwatch` (откатывается за
  центроид группы на `overwatchTrailDistance`), пока не перезарядится.

DEW исключён из `rest` в `assignIdleUnits` и из `mobilizeDefense` — им владеет
только его собственный проход. Скрипт переписывается **лишь при смене**
программы: иначе `guardPost` тянул бы rng-бросок каждый тик (а поток rng обязан
совпадать у пиров) и стирал бы `roamTarget`.

Побочная деталь, вскрытая тестом: DEW на `Guard` занимает слот гвардейской квоты
— так же, как это давно делает `ew`. Оставлено как есть: замороженного у базы
теперь добивают (см. предыдущий раздел), так что вклад в оборону реальный.
