...
## Постфактум — выстрел не читался на экране

Симптом: слышен только звук. Код-путь был цел (снаряд спавнится — иначе не было
бы и звука), проблема чисто в весе картинки: 3-пиксельная точка, живущая ~0.35 с,
и **полное отсутствие обратной связи в момент попадания**. У оружия без урона и
без взрыва попадание выглядело ровно как промах.

Три независимых канала вместо одного:

1. **Вспышка попадания** — новый `EffectKind` (`Blast` | `Emp`) в компоненте
   `Effect` и фабрика `spawnEmpBurst`. Переиспользует архетип `explosion`
   целиком: `explosionSystem` стареет его как обычно, `ExplosionView` выбирает
   картинку по `kind` — два разлетающихся кольца плюс радиальные дуги, а не
   заливка, чтобы не читалось как взрыв. Спавн вынесен **после** цикла запроса
   (как в `detonateBomb`) — добавлять сущности во время итерации нельзя.
2. **Снаряд** — болт вместо пули: ореол, бело-раскалённое ядро вдвое крупнее
   обычного и молниевый хвост, перерисовываемый каждый кадр.
3. **Индикатор паралича** — вместо статичной дуги разорванное кольцо с
   пробегающими по корпусу разрядами, перерисовка каждый кадр; корпус тускнеет
   до 0.45.

Канал (1) устойчив к любым проблемам слоя снарядов: он рисуется через тот же
`ExplosionView`, что и взрывы от смертей, которые заведомо работают.
