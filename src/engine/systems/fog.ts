 ...
 * grows (terrain is static, so remembered ground stays revealed). Bumps
 * `fog.version` whenever the mask changes so the renderer can skip redraws.
 * Player-only — the AI has no fog to draw.
 */
export function fogSystem(ctx: GameContext): void {
  ...
  for (let ty = 0; ty < height; ty++) {
    ...
    for (let tx = 0; tx < width; tx++) {
      ...
      const seen = scouts.some(
        (s) => distance(s.position!.x, s.position!.y, cx, cy) <= (s.sightRange ?? 0),
      );
      if (visRow[tx] !== seen) {
        visRow[tx] = seen;
        changed = true;
