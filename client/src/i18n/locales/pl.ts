import type { Dict } from '../dict';

export const pl: Dict = {
  mainMenu: {
    title: 'Drone Directive',
    difficulty: 'Poziom trudności',
    opponents: 'Przeciwnicy',
    opponentsHint: 'Każdy walczy z każdym',
    help: 'Pomoc',
    controls: 'Sterowanie',
    start: 'Start',
    language: 'Język',
    controlsTitle: 'Sterowanie',
    close: 'Zamknij',
    ctrlA: 'Zaznacz wszystkie roboty',
    esc: 'Pauza',
    dblClick: 'Na robocie — zaznacz wszystkie z tą samą bronią',
    dblClickBase: 'Na swojej bazie — otwórz „Budowa i dyrektywa”',
    selectBase: 'Zaznacz swoją bazę',
    setRally: 'Przy zaznaczonej bazie — ustaw punkt zbiórki',
    clearRally: 'Na zaznaczonej bazie — usuń punkt zbiórki',
    groupAssign: 'Zapisz obecne zaznaczenie jako grupę',
    groupSelect: 'Wybierz zapisaną grupę',
    droneHeading: 'Dron obserwacyjny',
    selectDrone: 'Na dronie — zaznacz go',
    sendDrone: 'Przy zaznaczonym dronie — wyślij go tam',
    landRelease: 'Przy zaznaczonym dronie — wsiądź na robota / wystartuj',
    serviceMenu: 'Uzbrój tryb eksperymentalny (robot zostanie zniszczony)',
    steerHull: 'Prowadź opanowany kadłub: W/S — ciąg, A/D — skręt',
    fireWeapon: 'Strzał z broni opanowanego robota',
    panView: 'Przesuwaj widok',
    units: 'Jednostki',
    unitGuide: 'Przewodnik po jednostkach',
    singleplayer: 'Gra jednoosobowa',
    matchSetup: 'Ustawienia rozgrywki',
    discord: 'Discord',
    desktopApp: 'Wersja na PC',
    quit: 'Zakończ grę',
  },
  difficulty: {
    easy: 'Łatwy',
    easyHint: 'SI rozwija się wolniej',
    normal: 'Normalny',
    normalHint: 'SI rozwija się w twoim tempie',
    hard: 'Trudny',
    hardHint: 'SI rozwija się szybciej',
  },
  mapSize: {
    label: 'Rozmiar mapy',
    small: 'Mała',
    smallHint: '40×40 — najszybsze mecze',
    medium: 'Średnia',
    mediumHint: '60×60 — zbalansowana (domyślna)',
    large: 'Duża',
    largeHint: '80×80 — najdłuższe, najbardziej taktyczne',
  },
  hud: {
    title: 'Drone Directive',
    command: 'Dowodzenie',
    units: 'Jednostki',
    directive: 'Dyrektywy',
    paused: 'Pauza',
    showDrone: 'Pokaż drona',
    showDroneHint: 'Przenieś kamerę do drona obserwacyjnego',
  },
  confirmExit: {
    title: 'Wyjść do menu głównego?',
    body: 'Mecz się na tym kończy — nie da się do niego wrócić. W meczu sieciowym przeciwnik zostanie powiadomiony, że gra się skończyła.',
    confirm: 'Wyjdź',
    cancel: 'Zostań',
  },
  gameOver: {
    victory: 'Zwycięstwo',
    defeat: 'Porażka',
    victoryBody: 'Wszystkie bazy wroga zniszczone.',
    defeatBody: 'Wszystkie twoje bazy zostały zniszczone.',
    mainMenu: 'Menu główne',
    playAgain: 'Zagraj ponownie',
  },
  buildRobot: {
    title: 'Budowa i dyrektywa',
    chassis: 'Podwozie',
    weapon: 'Broń',
    program: 'Dyrektywa',
    cost: 'Koszt',
    available: 'Dostępne',
    close: 'Zamknij',
    startAutoBuild: 'Włącz autoprodukcję',
    stopAutoBuild: 'Wyłącz autoprodukcję',
    preview: 'Wybrana konfiguracja',
    queueBack: 'Na koniec kolejki',
    queueBackHint: 'Trafia na koniec kolejki budowy',
    queueFront: 'Na początek kolejki',
    queueFrontHint: 'Wskakuje przed wszystkich czekających, ale nie przerywa już rozpoczętej budowy',
    atCap: 'Osiągnięto limit jednostek — nie można dodać do kolejki',
    queueHeading: 'W kolejce',
    cancelQueued: 'Usuń z kolejki',
  },
  statusPanel: {
    resources: 'Zasoby',
    queue: 'Kolejka',
    building: 'Budowa',
    waiting: 'Brak funduszy',
    auto: 'Auto',
    stop: 'Stop',
    buildProgram: 'Budowa',
    selection: 'Zaznaczenie',
    droneReadyToast: 'Nowy dron obserwacyjny czeka nad twoją bazą',
    droneReadyAction: 'Do drona',
    shield: 'Tarcza energetyczna',
    shieldHint: 'Jeden ładunek na mecz — kopułę można wznieść tylko raz',
    shieldUp: 'Kopuła aktywna',
    shieldSpent: 'Tarcza zużyta',
  },
  selection: {
    title: 'Zaznaczenie',
    all: 'Zaznacz wszystkie',
    clear: 'Wyczyść zaznaczenie',
    byWeapon: 'Wg uzbrojenia',
    droneHeading: 'Dron obserwacyjny',
    drone: 'Zaznacz drona',
  },
  programming: {
    selectUnits: 'Wybierz jednostkę/i, aby wydać dyrektywę.',
    enemyUnit: 'Jednostka wroga — nie można wydać dyrektywy.',
    robotsSelected: 'robotów zaznaczonych',
    weapon: 'Broń',
    baseSelected: 'Baza',
    baseProgram: 'Nowe jednostki',
    rallyPoint: 'Punkt zbiórki',
    rallyNone: 'nie ustawiony',
    rallyHint:
      'PPM na mapie — ustaw, gdzie zbierają się nowe jednostki z dyrektywami „Bezczynny” i „Obrona”; PPM na bazie — usuń punkt.',
    directiveHeading: 'Dyrektywa',
    formationHeading: 'Szyk',
  },
  serviceMenu: {
    title: 'Menu serwisowe',
    warning: 'Gdy czas trybu się kończy, robot zostaje zniszczony. Przytrzymaj, aby uzbroić.',
    shield: 'Tarcza',
    overload: 'Przeciążenie zagłuszacza',
    shieldNote: 'Pełna niewrażliwość przez cały czas — dość, by przejść przez ogień.',
    overloadNote: 'Na koniec impuls EMP: wszystko wrogie w zasięgu przestaje działać.',
    unavailable: 'Ten robot nie ma czego przeciążyć.',
    running: 'Tryb już działa.',
    holdToArm: 'Przytrzymaj',
    seconds: 's',
  },
  formations: {
    line: 'Linia',
    box: 'Czworobok',
    spread: 'Rozproszenie',
    none: 'Bez szyku',
    lineNote: 'Szeregi, działa z przodu. Najszerszy front — i na tyle ciasno, że zagłuszarka osłania wszystkich.',
    boxNote: 'Zwarty kwadrat: radar, zagłuszarka i kamikaze w środkowych polach. Szyk eskorty — i najbardziej wrażliwy na jeden wybuch.',
    spreadNote: 'Linia w odstępach większych niż promień wybuchu. Przetrwa kamikaze i nalot dronów, ale skrzydła wypadają poza zagłuszarkę.',
    noneNote: 'Rozejść się. Każda jednostka znów idzie sama.',
  },
  programs: {
    idle: 'Bezczynny',
    guard: 'Obrona',
    attackBase: 'Atak na bazę',
    attackRobots: 'Atak na roboty',
    scout: 'Zwiad',
    attackTarget: 'Atak celu',
    overwatch: 'Osłona',
    defendBase: 'Obrona bazy',
    groupAttack: 'Atak grupowy',
    none: 'Brak',
    intro: 'Dyrektywa to stały rozkaz: jednostka wykonuje go samodzielnie, dopóki nie przydzielisz innego.',
    guardNote:
      'Utrzymuje miejsce, w którym otrzymał rozkaz, patrolując wokół niego. Pod ostrzałem unika i odpowiada ogniem, ale nie podejmuje pościgu.',
    defendBaseNote:
      'Trzyma twoją bazę i sam wyrusza naprzeciw wszystkiemu, co się do niej zbliży, po czym wraca na posterunek. W przeciwieństwie do Obrony na jednego intruza reaguje cała linia, a nie tylko jednostka, która ma go w zasięgu.',
    attackBaseNote:
      'Naciera na najbliższą znaną drużynie bazę wroga, rozprawiając się z robotami po drodze. Dopóki bazy nie odnaleziono — przeszukuje mapę.',
    attackRobotsNote:
      'Poluje na wszystkie wykryte przez drużynę roboty wroga, potem przechodzi na ich bazę. Dopóki nic nie jest znane — przeszukuje mapę.',
    scoutNote:
      'Przemierza mapę i odsłania ją dla całej drużyny. Walczy tylko z tym, co samo wejdzie w zasięg — jego zadaniem jest znaleźć wroga, nie ścigać.',
    overwatchNote:
      'Trzyma się za nacierającą grupą albo blisko własnej bazy. Wycofuje się, gdy tylko oberwie — rola dla jednostek bez broni.',
  },
  chassis: {
    tracks: 'Gąsienice',
    wheels: 'Koła',
    legs: 'Nogi',
    statsHp: 'HP',
    statsSpeed: 'Prędkość',
    statsSight: 'Zasięg wzroku',
    tracksNote: 'Wytrzymałe i stabilne — dobre do utrzymywania pozycji i znoszenia obrażeń',
    wheelsNote: 'Szybkie, ale kruche — idealne do zwiadu i szybkich uderzeń',
    legsNote: 'Wolne, ale najbardziej wytrzymałe — stworzone do długich starć',
  },
  weapons: {
    none: 'Brak',
    cannon: 'Działo',
    missiles: 'Rakiety',
    bomb: 'Bomba',
    radar: 'Radar',
    ew: 'Zakłócacz',
    dew: 'DEW',
    fpv: 'FPV',
    statsRange: 'Zasięg',
    statsDamage: 'Obrażenia',
    statsReload: 'Przeładowanie',
    statsSalvo: 'Salwa',
    statsFlight: 'Lot',
    radarNote: 'Brak broni — podwaja zasięg wzroku',
    ewNote: 'Brak broni — zagłusza wzrok wrogów i strąca uderzeniowe drony FPV w promieniu',
    bombNote: 'Wybucha przy kontakcie, promień wybuchu',
    cannonNote: 'Zbalansowana szybkostrzelność i zasięg — solidny wybór uniwersalny',
    missilesNote:
      'Większy zasięg i obrażenia kosztem wolniejszego przeładowania. Jedyna broń zdolna zniszczyć cel powietrzny — drona obserwacyjnego lub nadlatującego drona uderzeniowego FPV',
    dewNote: 'Broń energii skierowanej: nie zadaje obrażeń — unieruchamia dowolny cel, naziemny lub powietrzny, na, s:',
    fpvNote:
      'Wysyła drony uderzeniowe w dowolny cel, który widzi twoja strona — na każdą odległość i ponad terenem, ale sam nosiciel nigdzie nie jedzie. Rój zatrzymuje obrona przeciwlotnicza i wrogi zakłócacz',
  },
  aria: {
    resume: 'Wznów',
    pause: 'Pauza',
    unmute: 'Włącz dźwięk',
    mute: 'Wycisz',
    volume: 'Głośność efektów',
    musicVolume: 'Głośność muzyki',
    soundSettings: 'Ustawienia dźwięku',
    directivesHelp: 'O dyrektywach',
    language: 'Język',
    theme: 'Motyw interfejsu',
    close: 'Zamknij',
    enterFullscreen: 'Pełny ekran',
    exitFullscreen: 'Wyjdź z pełnego ekranu',
    exitToMenu: 'Wyjdź do menu głównego',
  },
  theme: {
    command: 'Dowodzenie',
    crt: 'Terminal CRT',
  },
  sound: {
    title: 'Dźwięk',
    settings: 'Ustawienia dźwięku',
    effects: 'Efekty dźwiękowe',
    on: 'Wł.',
    off: 'Wył.',
    volume: 'Głośność efektów',
    music: 'Muzyka',
    musicVolume: 'Głośność muzyki',
  },
  graphics: {
    title: 'Grafika',
    settings: 'Ustawienia grafiki',
    quality: 'Jakość',
    high: 'Wysoka',
    medium: 'Średnia',
    low: 'Niska',
    hint: 'Niższe ustawienia renderują mniej pikseli — płynniej na słabszych maszynach, mniej ostro na dokładnych ekranach.',
    reload: 'Odśwież stronę, aby zastosować wygładzanie krawędzi.',
  },
  loading: {
    title: 'Wyjście na pozycje',
    sides: 'Strony',
    you: 'Ty',
    opponent: 'Przeciwnik',
    bot: 'Bot',
    tipLabel: 'Notatka z pola',
    tip1: 'Góry zatrzymują i pojazdy, i ogień. Krater to zagłębienie — nie da się przez niego przejechać, ale strzały przechodzą swobodnie.',
    tip2: 'Kopuła energetyczna to jeden ładunek na cały mecz. Zużyj go na atak, który inaczej by cię wykończył.',
    tip3: 'Dron obserwacyjny ląduje na wolnej maszynie i prowadzi ją ręcznie — broń strzela na twoją komendę, nie na decyzję autopilota.',
    tip4: 'Zestrzelony dron jest odbudowywany w bazie. Pasek gotowości w interfejsie pokaże, kiedy nowe oko wzbije się w powietrze.',
    tip5: 'Każda strona ma jeden limit robotów, wliczając zamówienia w kolejce. Wydawaj sloty na armię, której naprawdę wymaga twój plan.',
    tip6: 'Ustaw bazie punkt zbiórki — wszystko, co zbuduje, ruszy tam już ze swoją dyrektywą.',
  },
  online: {
    multiplayer: 'Tryb wieloosobowy',
    online2p: 'Online (2 graczy)',
    title: 'Online dla dwóch graczy',
    connecting: 'Łączenie…',
    shareCode: 'Wyślij ten kod pokoju przeciwnikowi:',
    copyCode: 'Kopiuj kod pokoju',
    codeCopied: 'Skopiowano!',
    waitingOpponent: 'Oczekiwanie na przeciwnika…',
    waitingPeer: 'Czekamy na przeciwnika…',
    reconnecting: 'Utracono połączenie — ponowne łączenie…',
    hostGame: 'Utwórz grę',
    createRoom: 'Utwórz pokój',
    joinGame: 'Dołącz do gry',
    roomCodePlaceholder: 'KOD POKOJU',
    joinRoom: 'Dołącz do pokoju',
    cancel: 'Anuluj',
    outdatedTitle: 'Ta wersja jest nieaktualna',
    outdatedBody: 'Gra sieciowa wymaga aktualnej wersji.',
  },
  update: {
    available: 'Dostępna jest nowa wersja',
    reload: 'Przeładuj',
    download: 'Pobierz',
    later: 'Później',
  },
  chat: {
    title: 'Czat',
    open: 'Czat z przeciwnikiem',
    close: 'Zwiń czat',
    leave: 'Opuść czat',
    placeholder: 'Wiadomość…',
    send: 'Wyślij',
    you: 'Ty',
    opponent: 'Przeciwnik',
    peerOnline: 'Przeciwnik jest online',
    peerAway: 'Przeciwnik jest offline',
    connecting: 'Ponowne łączenie…',
    empty: 'Brak wiadomości.',
    muteSound: 'Wycisz dźwięk nowych wiadomości',
    unmuteSound: 'Włącz dźwięk nowych wiadomości',
  },
  unitsGuide: {
    title: 'Przewodnik po jednostkach',
    intro: 'Robot składa się z podwozia (mobilność) i broni (siła ognia).',
    chassisHeading: 'Podwozie',
    weaponsHeading: 'Broń',
    baseHeading: 'Baza',
    baseNote:
      'Wbudowana wyrzutnia rakiet, jedna: baza broni się sama. Priorytet mają cele powietrzne — dron obserwacyjny w zasięgu dostaje ogień przed wszystkim, co jest na ziemi.',
  },
  device: {
    rotateTitle: 'Obróć urządzenie',
    rotateBody: 'W Drone Directive gra się w orientacji poziomej — pole bitwy potrzebuje szerokości.',
    tooSmallTitle: 'Mały ekran',
    tooSmallBody: 'Gra jest przystosowana do tabletu lub większego — część elementów może się nie zmieścić.',
    playAnyway: 'Zagraj i tak',
  },
};
