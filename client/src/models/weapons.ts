...
import { box, detail, plate, ring, seg, tube } from './primitives';
 ...
 * wireframe at range has room for fewer, so each model keeps only the first — at
 * tier 0. The workings that make a gun read as *that* gun close up — a muzzle brake,
 * a warhead in a tube, a feed horn — go under `detail(1, …)`, which the preview
 * panel asks for and the hull view never pays for.
 */
