...
- `screens/` — `MainMenu` (the title screen: `MenuNav` + `MatchSetupPanel`/`OnlinePanel`, one modal slot), `GlobalSettingsBar` (language/theme/sound/graphics/controls/fullscreen), `OnlinePanel`, `GameOverModal`. Pre-match base setup used to live here (auto-production + the new-robot directive) and was **removed from the menu**: too many knobs in front of a first-time player, for a thing the in-match build dialog already does. `settings.base` survives as the defaults a match starts from (`gameScene`, `GameApp.applyOnlineBaseSetup`) — nothing edits it any more.
...
- `common/` — `Button` (Headless UI-backed), `Bar`, and `Dialog`/`DialogFrame`. **Every modal's panel goes inside `DialogFrame`, never a bare `<div className="dialog-frame">`.** Headless UI closes on the `pointerup`/`touchend` outside the panel and React unmounts the dialog inside that event, so the *click* of the same gesture lands on whatever the modal was covering — the title screen's Start, a HUD button, the battlefield. `DialogFrame` swallows exactly that one click (window, capture phase). `MainMenu` keeps the raw div: it reuses the frame's geometry but is not a dialog.
